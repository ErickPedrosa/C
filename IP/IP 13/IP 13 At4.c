...
void ordena_estrutura(aluno*, uint32_t);
...
int main(int argc, char** argv) {
	...
	ordena_estrutura(ficha_alunos, num_alunos);
...
void imprime_media(aluno* ficha_a, uint32_t n_alunos) {
	...
	printf("\nA media geral da turma foi de %f\n", media_geral);

	return;
}

void ordena_estrutura(aluno* ficha_a, uint32_t n_alunos) {
	aluno ficha_copia;
	counter g, h;



	for (h = 0; h < n_alunos; h++) {
		for (g = 0; g < (n_alunos - 1); g++) {
			if (ficha_a[g].media > ficha_a[g + 1].media) {
				ficha_copia = ficha_a[g];
				ficha_a[g] = ficha_a[g + 1] ;
				ficha_a[g + 1] = ficha_copia;
			}
		}
	}
